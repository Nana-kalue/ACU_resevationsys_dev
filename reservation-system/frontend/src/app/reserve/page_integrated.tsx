...
export default function ReservePage() {
  ...
  const getWeekDates = (weekOffset: number) => {
    ...
    for (let i = 0; i < 7; i++) {
      ...
      dates.push({
        ...
        dayOfWeek: dayOfWeek,
        fullDate: date,
        isoString: date.toISOString().split('T')[0] // YYYY-MM-DD format
      });
    }
  ...
  const fetchAvailability = async (weekDates: any[]) => {
    ...
    try {
      ...
      for (const dateInfo of weekDates) {
        const response = await apiClient.getAvailability(dateInfo.isoString, selectedPlan.id);
        if (response.success && response.data) {
  ...
  const getAvailability = (date: string, time: string) => {
    ...
    if (!dateKey || !availability[dateKey]) return false;
      ...
      <div className="max-w-4xl mx-auto px-4">
        ...
        <div className="bg-white rounded-lg shadow-md p-8">
          ...
          <div className="mb-8">
            ...
            <select
              className="w-96 px-4 py-2 border border-gray-300 rounded-md bg-white font-medium"
