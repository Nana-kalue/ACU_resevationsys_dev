...
export default function SuspendPage() {
  ...
  const [currentMonth, setCurrentMonth] = useState(new Date().getMonth() + 1); // 現在の月
  ...
  const fetchBlockedSlots = async () => {
    ...
    try {
      ...
      response = await fetch(`${API_BASE_URL}/admin/blocked-slots?startDate=${startDate}&endDate=${endDate}`, {
        method: 'GET',
        headers: {
      ...
      if (data.success) {
        ...
        data.data.blockedSlots.forEach((block: any) => {
          ...
          const time = block.start_time.substring(0, 5); // 11:00:00 -> 11:00
          const key = `${day}-${time}`;

          // 現在表示中の月に含まれるブロックのみ処理
          if (blockDate.getFullYear() === currentYear && (blockDate.getMonth() + 1) === currentMonth) {
        ...
        // 既存の予約をbookedに設定
        try {
          const reservationResponse = await fetch(`${API_BASE_URL}/admin/reservations?startDate=${startDate}&endDate=${endDate}`, {
            method: 'GET',
            headers: {
              'Content-Type': 'application/json'
            },
      ...
      const errorMessage = error instanceof Error ? error.message : 'ブロック状況の取得に失敗しました';
      alert('ブロック状況の取得に失敗しました: ' + errorMessage);
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="space-y-6 max-w-full">
      ...
      <div className="bg-white rounded-lg shadow p-6">
        <div className="flex flex-wrap gap-8 text-sm">
          ...
          <div className="flex items-center gap-2">
            <div className="w-12 h-10 bg-yellow-100 border border-gray-300 flex items-center justify-center text-lg font-bold text-gray-900">
              ◎
            </div>
          ...
          {/* 2つ目 */}
          <div className="flex items-center gap-2">
            <div className="w-12 h-10 bg-white border border-gray-300 flex items-center justify-center text-lg font-bold text-gray-900">
              ○
            </div>
      ...
      {/* 月の選択 */}
      <div className="bg-white rounded-lg shadow p-6 max-w-full overflow-hidden">
        ...
        <div className="space-y-4">
          <div>
            ...
            <input
              ...
              className="px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 text-gray-900 font-medium"
            />
          </div>
